...
var knowCSS = knowCSSProto.prototype = {
...
function knowNow (hL) {
	...
	if (hA) {
		...
		var hE = (hL in knowOriginal) ? knowOriginal[hL] : hL.innerText;
		hE = knowEntities(hE.replace(/\n\t/gi, '\n'));
		hW.document.body.innerHTML = '<style>body{padding:0px;margin:0px;height:100%;width:100%;}textarea{box-sizing:border-box;font-family:verdana;font-size:12px;border:0px;padding:3px}</style><textarea style="width:100%; height:100%;" onKeyUp="window.opener.knowApply(this, \'' + hL + '\'); return true;" onkeydown="e=event; if ((e.keyCode==9) || (e.which==9)) { e.preventDefault(); var s = this.selectionStart; this.value = this.value.substring(0,this.selectionStart) + \'\\t\' + this.value.substring(this.selectionEnd); this.selectionEnd = s+1; }">' + hE + '</textarea>';
...
function knowApply(hA, hB) {
	var hC = knowLayer(hB);
	if (hC) {
		if (knowOriginal[hB] !== hA.value) {
			knowOriginal[hB] = hA.value;
			knowFindScripts(hC, hA.value);
			knowCSS(hB);
		}
	}
}
function knowFindScripts(dO, dT) {
	dO.innerHTML = dT;
	var dS = [];
	var dR = dO.childNodes;
	for (var i=0; dR[i]; i++) {
		if (dS && knowNode(dR[i], 'script') && (!dR[i].type || dR[i].type.toLowerCase() === 'text/javascript')) { dS.push(dR[i].parentNode ? dR[i].parentNode.removeChild(dR[i]) : dR[i]); }
	}
	for (var dA in dS) {
		if (typeof dA === 'string') { knowAddScripts(dS[dA]); }
	}
}
function knowNode(nE, nN) {
	return nE.nodeName && nE.nodeName.toUpperCase() === nN.toUpperCase();
}
function knowAddScripts(eL) {
...
// JAA TODO - target css to specific browsers - ex: safari prevent input zoom font-size-16px
function getBrowser () {
	// know="safari{} chrome{} firefox{} edge{} opera{} etc"
}
function getOS () {
	// know="mac{} win{} linux{} unix{} etc"
}
function getPlatform () {
	// know="ios{} android{}"
}
function getView () {
	// know="mobile{} tablet{} desktop{}"
}
...
// JAA TODO - add support for col-SCREEN and offset-SCREEN (ex: col-sm-1 offset-xl-3)
...
function getShortHand (classFound, classesFound) {
	...
	if (classFound in shortHand) {
		classFound = shortHand[classFound].trim();
...
function getValue(val) {
	if (val.indexOf('/') > -1) {
		...
		var vals = val.split(' ');
		if (vals.length > 2) {
			if (vals[2] in hexColors) {
				vals[2] = '#' + getShortColor(hexColors[vals[2]]);
				val = vals.join(' ');
			}
		}
	}
	...
	for (var key in classListCheck) {
		...
		while ((aM = zA.exec(grepTag)) !== null) {
			...
			if (dynamic.length > 0) { classList[screen + '_' + dynamic + '_' + action] = aM[2]; }
				...
				//modifiers = getModifiers(container, modifier);
				//containers = getContainers(container, containers);
						...
						for (var modifier in modifiers) {
							for (var i=0; i<actions.length; i++) {
								...
								for (var action in actionSet) {
									var containerPrefix = actionSet[action];
									if (screen in screenSizes) { screen = screenSizes[screen].join('-'); }
									//console.log(['here', screen in screenSizes, screen, modifier, action, container, containerPrefix]);
									// JAA TODO - test all these variations
									if (containerPrefix.length > 0) { classList[screen + '_' + containerPrefix + action + '_'] = aM[2]; }
									else if (container.indexOf('media-') == 0 || screenTypes.includes(container) || ['font-face'].includes(container)) { classList[container + '_' + modifier + '_' + action] = aM[2]; }
								}
							}
						}
					}
...
function getColor (hE, hC) {
	if (hC.indexOf('background') > -1 || hC.indexOf('color') > -1) {
		...
		var hF = hE.replace('#','');
		if (hF in hexColors) { hF = hexColors[hF].trim(); }
...
function getLocalMixins () {
	if (typeof localMixins !== 'undefined') {
		for (var key in localMixins) { allMixins[key] = getVariables(localMixins[key]); }
	}
}
	...
	if (html.indexOf('$') > -1) {
		...
		while ((varKey = zZ.exec(html)) !== null) {
			...
			if (typeof cssVars === 'object') {
				if (varKey[1] in cssVars) { varFound = cssVars[varKey[1]].replace('/\\\\/gis', ''); }
			}
...
var usedClass = {};
var letters = "abcdefghijklmnopqrstuvwxyz" + "abcdefghijklmnopqrstuvwxyz".toUpperCase();
...
/*
function getClassName (classTotal) {
	var aA = letters + letters.toUpperCase();
	var lA = aA.length;
	var lB = lA * lA;
	var lC = lA * lA * lA;
	var aC = "";
	...
	var aZ = a0;
	if (a0 >= lC) {
		var a0 = Math.floor(a0/lC);
		var a1 = aZ - (a0 * lC);
		aZ = a1;
		var a2 = 0;
		var a3 = 0;
		if (a1 >= lB) {
			var a1 = Math.floor(a1/lB);
			var a2 = aZ - (a1 * lB);
			aZ = a2;
			if (a2 >= lA) {
				a2 = Math.floor(a2/lA);
			...
			var a1 = Math.floor(a1/lA);
			var a2 = aZ - (a1 * lA);
		}
		aY = [a0-1, a1, a2, a3];
	}
	else if (a0 >= lB) {
		var a0 = Math.floor(a0/lB);
		var a1 = aZ - (a0 * lB);
		aZ = a1;
		var a2 = 0;
		if (a1 >= lA) {
...
function knowCSSRender (uI, uO) {
	var uX = {
		'minifyhtml': false,
		'minifycss': false,
		'classes': 'random', // sequential or random or detail
		'normalize': false,
		//'mobilefirst': true,
		//'mobilelast': true
	};
	...
	var classTags = document.querySelectorAll("[know]");
	getLocalMixins();
	for (var ii=0; ii<classTags.length; ii++) {
		classesHere = [];
		classList = {'none_none_none': getMixins(getVariables(classTags[ii].getAttribute("know")))};
			...
			if (classesFound.length > 0) {
				...
				for (var i=0; i<classesFound.length; i++) {
					...
					if (classFound.length > 0) {
						...
						if (className in shortHand) { className = shortHand[className]; }
...
//modules.export.knowCSS = knowCSS;
